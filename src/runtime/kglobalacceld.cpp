...
{
    const QString uniqueId = actionId.at(KGlobalAccel::ComponentUnique);

    // If a component for action already exists, use that...
    if (Component *c = m_registry->getComponent(uniqueId)) {
        return c;
    }

    // ... otherwise, create a new one
    const QString friendlyId = actionId.at(KGlobalAccel::ComponentFriendly);
    if (uniqueId.endsWith(QLatin1String(".desktop"))) {
        auto *actionComp = m_registry->createSAComponent(uniqueId, friendlyId);
        Q_ASSERT(actionComp);
        actionComp->activateGlobalShortcutContext(QStringLiteral("default"));
        actionComp->loadFromService();
        return actionComp;
    } else {
        auto *comp = m_registry->createComponent(uniqueId, friendlyId);
        Q_ASSERT(comp);
        return comp;
    }
}
