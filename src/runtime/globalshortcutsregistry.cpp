...
KServiceActionComponent *GlobalShortcutsRegistry::createSAComponent(const QString &uniqueName, const QString &friendlyName)
{
    auto it = findByName(uniqueName);
    if (it != m_components.cend()) {
        Q_ASSERT_X(false, //
...
void GlobalShortcutsRegistry::loadSettings()
{
    ...
    for (const QString &groupName : groupList) {
        ...
        Component *component = isDesktop ? createSAComponent(groupName, friendlyName) //
    ...
    for (const QString &file : desktopFiles) {
        ...
        auto *actionComp = createSAComponent(fileName, deskF.readName());
