...
QString makeUniqueName(KService::Ptr service)
{
...
void KServiceActionComponent::emitGlobalShortcutPressed(const GlobalShortcut &shortcut)
{
    ...
    auto launchWithToken = [this, shortcut](const QString &token) {
        ...
        if (m_service->property(QStringLiteral("DBusActivatable"), QVariant::Bool).toBool()) {
            QString method;
            const QString objectPath = QStringLiteral("/%1").arg(m_service->desktopEntryName()).replace(QLatin1Char('.'), QLatin1Char('/'));
            const QString interface = QStringLiteral("org.freedesktop.Application");
            QDBusMessage message;
            if (shortcut.uniqueName() == QLatin1String("_launch")) {
                message = QDBusMessage::createMethodCall(m_service->desktopEntryName(), objectPath, interface, QStringLiteral("Activate"));
            } else {
                message = QDBusMessage::createMethodCall(m_service->desktopEntryName(), objectPath, interface, QStringLiteral("ActivateAction"));
    ...
    if (KWindowSystem::isPlatformWayland()) {
        KWindowSystem::requestXdgActivationToken(nullptr, 0, m_service->desktopEntryName());
